...
def follow_up_3(browser, compte, total_li):
    print(f"lancement du fichier Follow up 3 pour le compte {compte.linkedin_account}")
    ...
    for x in total_li:
        ...
        try : 
            ...
            if (profile.message_sent==True) and (profile.follow_up_1==True) and (profile.follow_up_2==True) and (profile.follow_up_3==False) and (profile.replied==False) :
                ...
                if jours > 3 :
                    ...
                    Message = compte.follow_up_3
                    message_follow_up_2 = compte.follow_up_2
                    if '{first_name}' in Message:
                        Message = Message.replace('{first_name}', profile.first_name)
                    if '{first_name}' in message_follow_up_2:
                        message_follow_up_1 = message_follow_up_2.replace('{first_name}', profile.first_name)
                    ...
                    if [last_contenu_message] != [message_follow_up_2]:
                    ...
                    sauvegarde_followup3(profile=profile, compte=compte)
        except:
            pass
